...
const userHome = async (req, res, next) => {
    ...
    let proPop = await orderModel.find().populate("items.productId");
    console.log(proPop, 111);
    let topPro = await orderModel.aggregate([
        { $unwind: "$items" },
        { $group: { _id: "$items.productId", sum: { $sum: "$items.quantity" } } },
        { $sort: { sum: -1 } },
        { $group: { _id: null, topSelling: { $push: "$_id" } } },
    ]);
    console.log(topPro, 111111111);
